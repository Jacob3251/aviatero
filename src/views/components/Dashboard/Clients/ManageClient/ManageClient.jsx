...
function ManageClient() {
  ...
  const [data, setData, metaData, clientLoading] = useClient(currentPage);
  ...
  const handleDelete = async (item) => {
    ...
    if (confirm) {
      const { status } = await axios.delete(
        `https://consultancy-crm-serverside-1.onrender.com/api/client/${item.id}/delete`,
        {
          headers: {
          ...
          <div className="px-5">
            {data.length !== 0 ? (
              <div className="relative overflow-x-auto shadow-md sm:rounded-lg border-primary border-2 p-5">
                <div className=" flex items-center justify-end flex-column  md:flex-row space-y-4 md:space-y-0 space-x-2 pb-4  pt-3">
                      ...
                      >
                        ...
                        <td className="px-6 py-4 flex space-x-2 text-[24px]">
                          ...
                          <div
                            onClick={() =>
                              navigate(
                                `/dashboard/clients/information/${item.id}`
                              )
                            }
